...
const GamblingWheel = ({ onBack }) => {
  ...
  const spinWheel = async () => {
    ...
    // Ensure we rotate forward by adding 360 if negative
    if (rotationNeeded < 0) {
      rotationNeeded += 360;
    }

    // Add the base spins and the calculated rotation
    const finalRotation = rotation + (Math.floor(spins) * 360) + rotationNeeded;
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 bg-[#0f0f0f] p-4 pt-12 overflow-y-auto z-40"
    >
